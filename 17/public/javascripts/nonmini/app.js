      ...
      $('.editable').each(function(){
       ...
       var parsecall = function(){        
        ...
        $.post( "/parse", { ticket: currentText }).done(function( data ) {
             var new_data = $("#autocomplete1").html().toString().replace(currentText,data);                    
             //alert(JSON.stringify(data));
             console.log(data);
            // if (!processed($ticket_process_stats,latest_q)){
              // $ticket_process_stats.push({matched: latest_q,parsed: true,render: true });
              // var matching_value_in_queue = $ticket_process_stats.filter(function (query){ 
              //                                                              var new_data = $("#autocomplete1").html().toString().replace(currentText,data);                                 
               //                                                             console.log("OLD DATA: " + query.matched + " NEW DATA: " + new_data + " LEVENSHTEIN " + levenshtein(query.matched,new_data));

                                                                         //   return ((levenshtein(query.matched,new_data)>=(new_data.length-query.matched.length)) &&
                                                                         //           (query.parsed) &&
                                                                         //           (query.render));
                                                                         // });
               ...
               //if (matching_value_in_queue.length==0){
                 $("#autocomplete1").html(new_data);
