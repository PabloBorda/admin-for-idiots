...
# http://myhub.corp.ebay.com/User%20Photos/Profile%20Pictures/_w/corp_mmather_SThumb_jpg.jpg
  ...
  get '/suggest_me' do
    ...
    coll.find({ "$text" => { "$search" => ("\"" + findstr +  "\" ") } }).limit(15).each do |p|
      ...
      people = ["pborda","okeskin","mlaskowski","anegro","vconradt","mfagot","cplanchon","juobrien","pgaskin","ankingston"]
      pic = "http://myhub.corp.ebay.com/User%20Photos/Profile%20Pictures/_w/corp_" + people[rand(people.size)].to_s + "_MThumb_jpg.jpg"
      @suggestions.push ({ :q => findstr ,:mr => p["mr"] ,:knows => p["knows"], :for => p["knows"], :and_these_guys_agreee => ["vconradt","mfagot","cplanchon","juobrien","pgaskin"] })
    end
