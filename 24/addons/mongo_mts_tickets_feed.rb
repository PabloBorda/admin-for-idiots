...

{
  ...
  :helper_methods_exposed_to_url => [{:find_ticket_feed => Proc.new {
                                                            ...
                                                            #   if (session[:feed_sessions].size > 0)
                                                            #     coll = session[:feed_sessions].select {|s| (s[:name].eql? "mongo_mts_tickets_feed") and (!s[:session].nil?)}.first[:session]['mts-tickets-indexed-by-paragraph']                                                                                                                              
                                                            #   else
                                                            #     puts "SESSION NOT OPEN!! "
                                                            #   end
                                                            # else
                                                             client = Mongo::MongoClient.new('localhost', 27017).db('pborda', :strict => false)
